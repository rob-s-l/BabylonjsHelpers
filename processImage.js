...
/* ***** entry point heightMapfromImageData(image, scene, newWidth=40, newHeight=40, maxDepth = 1.0)  
   *****  creates a heightmap from coloured image by making it gray image and then calculating vectorpoints,   
   *****  heights, normals and uv data
   return custommesh with orifa=ganal image as texture
   parameter newwidth and newHeight for ersampling imagedata
   maxDepth is max height of map
   ****** */

 
export async function heightMapfromImageData(image, scene, newWidth=40, newHeight=40, maxDepth = 1.0) {
    let newHeightMapTexture = null;
    let imageData = null;

    //process the image for use and redtrieve data for texture and 
   ...
   // console.log("heights computed");
    ...
    // not using newWidth and height Values but use original imagesize as reference Values
